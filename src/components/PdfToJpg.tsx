...
export default function PdfToJpg() {
    ...
    const runExtraction = async () => {
        ...
        setProgress({ current: 0, total: 100, message: 'Extracting pages' });
        ...
        try {
            ...
            setProgress({ current: 0, total: numPages, message: 'Extracting pages' });

            const zip = new JSZip();
            const imagesFolder = zip.folder("Extracted_Pages");

            for (let i = 1; i <= numPages; i++) {
                if (abortRef.current) throw new Error('Extraction cancelled by user');
                ...
                setProgress({ current: i, total: numPages, message: 'Extracting pages' });
            }

            setProgress({ current: 0, total: 100, message: 'Compressing ZIP' });

            // Build ZIP
            const zipBlob = await zip.generateAsync({ type: 'blob' }, (metadata) => {
                setProgress({ current: Math.round(metadata.percent), total: 100, message: 'Compressing ZIP' });
            });
            ...
            console.error(e);
            if (e.message !== 'Extraction cancelled by user') {
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 h-full">
                ...
                {!file ? (
                    ...
                    <div className="flex-1 p-6 md:p-8 flex flex-col items-center justify-center bg-gradient-to-b from-transparent to-zinc-900/40">
                        <div className="p-8 bg-zinc-900 rounded-3xl border border-zinc-700/50 shadow-2xl flex flex-col items-center max-w-sm w-full relative group text-center">
                            ...
                            <button
                                onClick={isProcessing ? cancelExtraction : clearAll}
                                className="absolute top-4 right-4 z-10 p-2 bg-black/40 hover:bg-red-500/20 text-zinc-400 hover:text-red-400 rounded-xl transition-all"
                                title={isProcessing ? "Cancel extraction" : "Clear file"}
                            >
                            ...
                            {progress ? (
                                <>
                                    ...
                                    <h4 className="font-bold text-xl text-zinc-100 truncate w-full px-4 mb-2">
                                        {progress.message === 'Compressing ZIP' ? 'Compressing...' : 'Extracting...'}
                                    </h4>
                                    <div className="px-3 py-1 bg-zinc-800 rounded-full border border-zinc-700 mb-6">
                                        <p className="text-zinc-400 text-sm font-medium">
                                            {progress.message === 'Compressing ZIP' ? `${progress.current}% Complete` : `Page ${progress.current} of ${progress.total}`}
                                        </p>
                                    </div>
            ...
            <div className="w-full md:w-80 bg-zinc-900/50 rounded-2xl border border-zinc-800/80 p-6 flex flex-col">
                ...
                <div className="mt-6 pt-5 border-t border-zinc-800/80 text-[10px] text-zinc-500/80 leading-relaxed mb-6 font-medium">
